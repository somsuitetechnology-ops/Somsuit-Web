  ...
  const sendMessage = async () => {
    ...
    if (wantsHuman) {
      ...
      setTimeout(() => {
        const humanResponse: Message = {
          role: 'assistant',
          content:
            "We can connect you with our team. Use the button below to open our contact page and send a message.",
        };
    ...
    setMessages((prev) => [
      ...prev,
      {
        role: "assistant",
        content:
          "Thanks for your message. Use the button below to reach us on our contact page—we'll get back to you there.",
      },
    ]);
    setIsLoading(false);
  };
        ...
        <Card className="fixed bottom-20 sm:bottom-24 right-2 sm:right-6 z-40 w-[calc(100vw-1rem)] sm:w-[400px] max-w-[400px] h-[500px] sm:h-[550px] flex flex-col shadow-2xl border-2 border-accent/30 overflow-hidden animate-scale-in bg-card backdrop-blur-xl">
              ...
              <div key={idx}>
                ...
                {msg.role === "assistant" && /button below/i.test(msg.content) && (
                  <div className="flex justify-start mt-2 animate-fade-in">
                    <Button
                      onClick={openContact}
                      className="bg-accent hover:bg-accent-hover text-primary flex items-center gap-2 shadow-lg"
                    >
                      Go to contact
                    </Button>
                  </div>
          ...
          <div className="border-t border-border p-4 bg-background">
            ...
            <p className="text-xs text-muted-foreground mt-2 text-center">
              For detailed help, use our contact page
            </p>
          </div>
        </Card>
      )}
